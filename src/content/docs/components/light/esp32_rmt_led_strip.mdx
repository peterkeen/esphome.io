...
- **rgbw_order** (*Optional*, string): The order of the RGBW channels. Specify each of `R`, `G`, `B`, and `W`
  exactly once. This option cannot be used with `rgb_order`, `is_rgbw`, or `is_wrgb`.
